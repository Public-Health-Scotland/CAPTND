...
create_error_report <- function(dataset_choice = c("CAMHS", "PT")){
  ...
  for (hb in hb_vector){
    ...
    wb <- loadWorkbook("../../../report_templates/data_quality/error_report_template_updates.xlsx")
    ...
    paras <- c(paste0("Tab 1: Attended appointment records recieved in ", month_name, " with no attendance status"),
               ...
               paste0("Tab 5: Impossible case closed dates received in ", month_name),
               paste0("Tab 6: Cancelled appointments received in ", month_name, " with no cancellation date"),
