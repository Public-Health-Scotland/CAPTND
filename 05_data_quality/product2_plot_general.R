...
product2_plot_general <- function(df_rtt_plot_prep, date_max) {
  ...
  p <- df_rtt_plot_prep %>% 
    ...
    labs(title=paste0("Percentage of pathways where RTT is possible until ", date_max),
         fill='RTT status', 
         x='health board',
         y='% pathways') +
    theme_minimal()+
  ...
  pname=paste0(opti_report_dir,'/product2_general_', date_max,
               '.html')
