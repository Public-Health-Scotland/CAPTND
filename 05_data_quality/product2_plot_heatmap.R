...
product2_plot_heatmap <- function(df_rtt, date_max){
  ...
  df_rtt_plot_prep_perc <- df_rtt %>%
    ...
    #inner_join(pms, by = c(hb_name_o, dataset_type_o)) # no longer need to join in PMS info
  
  # duplicates rows, to allow plotting of PMS in geom_tile below using dummy varibale 'a'
  ...
  product2_plot_heatmap <- df_rtt_plot_prep_perc %>% 
    ggplot(aes(y = factor(!!sym(hb_name_o), levels = rev(level_order)), x = a, fill = traffic_light)) + 
