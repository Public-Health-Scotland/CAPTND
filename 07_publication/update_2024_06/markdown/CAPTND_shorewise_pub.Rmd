...
`r ifelse(dataset_choice == "CAMHS", "NB: CAMHS is available for children and young people aged 0-18 in Scotland (for those who meet referral criteria). In some cases, however, individuals over 18 may access CAMHS.", "")`
</Div>
...
<Div custom-style = "Bullet 1">
There were a total of `r table_data3[[15,2]]` `r dataset_choice` appointments
...
referred to `r dataset_choice` recorded in CAPTND. This was 
...
\newpage
<Div custom-style = "Table or chart caption">Table 1: Total referrals to `r dataset_choice` by health board, by quarter from `r date_label_5qt` to `r date_label` ^1^</Div>
...
## Distribution of `r dataset_choice` referrals by age and sex

Figure 1 shows the age distribution and biological sex of people referred to
`r dataset_choice` `r ifelse(dataset_choice == "PT", "services", "")` across NHSScotland
between `r date_label_5qt` and `r date_label`. There were `r agesex_total[[1,3]]`
...
In the quarter ending `r date_label`, `r dataset_choice` across NHSScotland
...
<Div custom-style = "Table or chart caption">Figure 1: Age distribution of `r dataset_choice` referrals by sex, NHSScotland, from `r date_label_5qt` to `r date_label`</Div>
```{r Referrals sex & age chart, fig.width=7.18, fig.height=3.31, include = TRUE}

knitr::include_graphics(paste0("//PHI_conf/MentalHealth5/CAPTND/CAPTND_shorewise/output/analysis_", data_analysis_latest_date, "/shorewise_publication/data/referrals/referrals_sex_age_", dataset_choice, ".png"))

```

## Distribution of `r dataset_choice` referrals by SIMD quintile

Figure 2 presents the number of people referred to `r dataset_choice`
`r ifelse(dataset_choice == "PT", "services", "")` across Scotland by SIMD
quintile, in the quarter ending `r date_label`. `r figs_simd_refs[[1,5]]`
(`r figs_simd_refs[[1,6]]`%) individuals referred to `r dataset_choice`
...
This demonstrates that a higher demand for `r dataset_choice` `r ifelse(dataset_choice == "PT", "services", "")`
exists in areas of higher deprivation.  

\newpage
<Div custom-style = "Table or chart caption">Figure 2: Total referrals to `r dataset_choice` by SIMD quintile, NHSScotland, quarter ending `r date_label`</Div>
```{r Referrals SIMD chart, fig.width=7.18, fig.height=3.31, include = TRUE}

knitr::include_graphics(paste0("//PHI_conf/MentalHealth5/CAPTND/CAPTND_shorewise/output/analysis_", data_analysis_latest_date, "/shorewise_publication/data/referrals/ref_simd_plot_last_qt_", dataset_choice, ".png"))
...
"As CAPTND is a dataset under development, the increase since the same quarter in the previous year may reflect improving data quality, rather than an actual increase in the referral acceptance rate."
}`  
...
In `r dataset_choice` the primary reason
for referral non-acceptance was recorded as '`r ref_rej_reasons[[1,4]]`'
...
`r ifelse(ref_rej_reasons[[2,4]] == "Unsuitable", "- meaning that the patient is unsuitable for the service to which they have been referred", "")` (`r ref_rej_reasons[[2,7]]`%). **A patient may be deemed unsuitable for `r dataset_choice` because their needs would be better met in another branch of mental health services, e.g. `r ifelse(dataset_choice == "CAMHS", "neurodevelopmental services", "psychiatry")`. MOVE AS NEEDED**  
...
Table 3 shows the total number of appointments made within `r dataset_choice` and recorded 
...
Of the `r figs_apps$"Total appointments"` `r dataset_choice` appointments in the
...
to treatment start time. The `r dataset_choice` DNA rate across NHSScotland in the
...
referrals to `r dataset_choice` were submitted to CAPTND, however,
...
<Div custom-style = "Table or chart caption">Table 4: Difference between submitted referral counts and CAPTND pathway referral counts, in `r dataset_choice` by health board, quarter ending `r date_label` ^1,2^</Div>
...
`r dataset_label` (`r dataset_choice`) Report (`r date_label_5qt` - `r date_label`)
...
collection methods compared with `r dataset_choice` waiting times statistics, as
