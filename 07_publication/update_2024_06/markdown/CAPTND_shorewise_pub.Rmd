...
figs_referrals <- table_data |> 
  ...
  mutate(diff_qt = `Mar '24` - `Dec '23`,
         diff_yr = `Mar '24` - `Mar '23`,
         prop_qt = round(100-(`Dec '23`/`Mar '24`*100), 1),
         prop_yr = round(100-(`Mar '23`/`Mar '24`*100), 1),
...
This publication of statistics in development presents the number of 
**individuals referred** to `r dataset_label` (`r dataset_choice`) from the Child, 
Adolescent, and Psychological Therapies National Dataset (CAPTND), at NHS Board 
and Scotland-level. Here ‘Scotland-level’ refers to the aggregated regional NHS 
Board figures that are available in CAPTND.  
...
CAPTND. Some data items, namely ‘UCPN’, ‘UPI’, and ‘CHI’, are required for an 
individual to be counted within CAPTND (see [Variables Reported](#variables-reported) section below). 
Due to this incomplete data, the absolute values reported are underestimating true 
activity but this is expected to improve over time through ongoing efforts to increase 
completeness and quality of CAPTND data returns.  
...
January 2020 to enable reporting to commence thereafter. Consequently, it is 
currently not possible to compare the CAPTND data presented to previous years.  
...
For the quarter ending `r date_label`, there were a total of `r figs_referrals[1,3]`
