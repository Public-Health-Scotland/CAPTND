...
figs_ref_acc <- read_parquet(
  ...
  select(-c("total", "prop")) |> 
  pivot_wider(names_from = quarter_ending, values_from = count) |> 
...
figs_apps_qt <- read_parquet(
  paste0("//PHI_conf/MentalHealth5/CAPTND/CAPTND_shorewise/output/analysis_", data_analysis_latest_date, "/shorewise_publication/data/appointments_att/apps_att_qt_hb.parquet")) |> 
   filter(dataset_type == dataset_choice,
         hb_name == "NHS Scotland") |> 
  group_by(hb_name, app_quarter_ending) |> 
  slice(1)#WIP
...
`r ifelse(dataset_choice == "CAMHS", "NB: CAMHS is available for children and young people aged 0-18 in Scotland (for those who meet referral criteria). In some cases, however, individuals over 18 may exist within the CAMHS data, and these records have not been removed on the basis of recorded age.", "")`
</Div>
...
in the same quarter ending `r date_label_yr`. </Div>
...
referred to `r dataset_choice`, compared to `r figs_referrals$last_yr` in the quarter ending
`r date_label_yr`.
...
<Div custom-style = "Table or chart caption">Table 1: Total referrals to `r dataset_choice` by health board, `r date_label_15m` to `r date_label` ^1^</Div>
...
For Scotland overall, of `r figs_basic_opti[[1,1]]` records,
`r figs_basic_opti$"Percent difference"` of submitted referral pathways could
not be counted in CAPTND. At the level of NHS boards, however, this varies
considerably with several boards having all of their referral pathways
counted in CAPTND. In health boards for which the pathway count is lower, this tends
to be due to issues around the health board's ability to record all of the required
identifiers, which we expect to improve over time with ongoing data quality
