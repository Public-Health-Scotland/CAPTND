...
summarise_non_acceptance_reason <- function(df){
  ...
  # by age
  df_all_hb_age <- df |> 
    group_by(dataset_type, hb_name, age_at_ref_rec, age_group, ref_rej_reason_desc) |> 
    summarise(count = n(), .groups = "drop") |>
    group_by(dataset_type, age_at_ref_rec, age_group, ref_rej_reason_desc) %>% 
    bind_rows(summarise(.,
                        across(where(is.numeric), sum),
                        across(hb_name, ~"NHS Scotland"),
                        .groups = "drop")) |> 
  ...
  df_month_hb_age <- df |>
    group_by(referral_month, dataset_type, hb_name, age_at_ref_rec, age_group, ref_rej_reason_desc) |>
    summarise(count = n(), .groups = "drop") |>
    group_by(referral_month, dataset_type, age_at_ref_rec, age_group, ref_rej_reason_desc) %>%
    bind_rows(summarise(.,
                        across(where(is.numeric), sum),
                        across(hb_name, ~"NHS Scotland"),
                        .groups = "drop")) |>
