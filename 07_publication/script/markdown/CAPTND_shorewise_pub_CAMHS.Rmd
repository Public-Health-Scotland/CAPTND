...
to the [official aggregate CAMHS collection and waiting times](https://publichealthscotland.scot/publications/child-and-adolescent-mental-health-services-camhs-waiting-times/child-and-adolescent-mental-health-services-camhs-waiting-times-quarter-ending-september-2024/).  
...
within which multiple related ethnic backgrounds are defined.
</Div>
...
<Div custom-style = "Bullet 1">
There were a total of `r figs_apps_qt$current_qt` CAMHS appointments
...
<Div custom-style = "Bullet 1">
Of the total `r ifelse(dataset_choice == "CAMHS", "CAMHS", "PT")` appointments recorded across 
NHSScotland, `r figs_tot_apps[[1,2]]` had an attendance status of 'Did Not Attend (DNA)' 
...
they may not meet the national referral criteria for CAMHS. Please see the 
...
CAPTND extract `r data_analysis_latest_date`
...
## Distribution of CAMHS referrals by ethnic group

Figure 3 shows the proportion of individuals referred to CAMHS identifying as belonging
to one of six ethnic groups. Each of these groups is made up of multiple related
ethnic identities (see [Variables Reported](#variables-reported) for further information). The
‘`r figs_eth_refs[[1,2]]`’ category accounted for `r figs_eth_refs[[1,6]]`% of
known responses (`r figs_eth_refs[[1,5]]` referrals). The 2022 Scotland census
found that 92.9% of Scotland’s population identified as belonging to the
‘White’ ethnic group (see [ScotPHO](https://www.scotpho.org.uk/population-groups/ethnic-minorities/data/population-composition/)).  

Of the remaining referral records, `r figs_eth_refs[[2,6]]`% identified with an
ethnicity within the ‘`r figs_eth_refs[[2,2]]`’ group; `r figs_eth_refs[[3,6]]`%
endorsed the ‘`r figs_eth_refs[[3,2]]`’ group; `r figs_eth_refs[[4,6]]`% identified
with an ethnicity in the ‘`r figs_eth_refs[[4,2]]`’ group;  `r figs_eth_refs[[5,6]]`%
identified with an ethnicity in the ‘`r figs_eth_refs[[5,2]]`’ group; and `r figs_eth_refs[[6,6]]`%
identified with an ethnicity in the ‘`r figs_eth_refs[[6,2]]`’ group. The proportion
of referrals where the ethnic group was ‘not known’ was high, at `r eth_not_known$count`
(`r eth_not_known$prop`%).  

With a high proportion of
‘not known’ responses for ethnicity in CAPTND it is not possible to compare the distribution
of referrals by ethnic group to the corresponding ethnic group in the overall Scottish population. While it may be that those belonging to minority ethnic groups might be less willing to disclose their ethnicity, it is not possible to speculate regarding the ethnic composition of those in the 'not known' category.

<Div custom-style = "Table or chart caption">Figure 3: Total referrals to CAMHS by ethnic group, NHSScotland, quarter ending `r date_label`</Div>
...
Figure 4 shows the looked after child status of individuals referred to CAMHS services
...
<Div custom-style = "Table or chart caption">Figure 4: Looked after child status of CAMHS referrals, NHSScotland, quarter ending `r date_label`</Div>
...
Figure 5 shows the Child Protection status of individuals referred to CAMHS services
...
<Div custom-style = "Table or chart caption">Figure 5: Child protection status of CAMHS referrals, NHSScotland, quarter ending `r date_label`</Div>
...
`r figs_ref_acc$last_yr`%. This shows that the referral acceptance rate across
CAMHS has remained fairly stable over time.  
...
Figure 6 below shows the reported reason for referral non-acceptance in NHSScotland
...
<Div custom-style = "Table or chart caption">Figure 6: Reasons for non-accepted referrals, NHSScotland, quarter ending `r date_label`</Div>
...
Figure 7 shows the actions taken by services following referral non-acceptance in
...
<Div custom-style = "Table or chart caption">Figure 7: Actions taken following non-accepted referrals, NHSScotland, quarter ending `r date_label`</Div>
...
Figure 8 shows the source of referrals for people referred to CAMHS services across
NHSScotland for the quarter ending `r date_label`. The most common referral source
to CAMHS services was '`r ref_source[[1,4]]`' (`r ref_source[[1,7]]`%), followed
by '`r ref_source[[2,4]]`' (`r ref_source[[2,7]]`%).  

<Div custom-style = "Table or chart caption">Figure 8: Most common sources of CAMHS referrals, NHSScotland, quarter ending `r date_label`</Div>
...
Table 3 shows the total number of appointments recorded in CAPTND for CAMHS in
...
[data files]`r paste0("(https://publichealthscotland.scot/publications/child-adolescent-and-psychological-therapies-national-dataset-captnd/", dataset_choice, "-in-captnd-4-march-2025/)")`.  
...
total appointments in the quarter ending `r figs_apps_qt$current_qt`, the overall DNA rate was `r figs_tot_apps[[1,3]]`
...
Figure 9 shows the most common locations in which CAMHS appointments were conducted
across NHSScotland for the quarter ending `r date_label`. The most common contact
location recorded was '`r appt_loc[[1,3]]`' (`r appt_loc[[1,7]]`%). The next most common known
contact location for CAMHS appointments was '`r appt_loc[[3,3]]`' (`r appt_loc[[3,7]]`%).
...
<Div custom-style = "Table or chart caption">Figure 9: Most common contact locations for CAMHS appointments, NHSScotland, quarter ending `r date_label`</Div>
...
Figure 10 shows the most common professional groups conducting CAMHS appointments
...
<Div custom-style = "Table or chart caption">Figure 10: Most common professional groups conducting CAMHS appointments, NHSScotland, quarter ending `r date_label`</Div>
...
Table 4 shows the total number first contact appointments made within CAMHS and
...
Of the `r figs_apps_qt$current_qt` total appointments in CAMHS in the
...
Figure 11 shows the first contact DNA rate over the last 15 calendar months, from
`r date_label_5qt` to `r date_label`, for each health board region (see [Glossary](#glossary)
section for more information).  

The first contact DNA rate for CAMHS appears reasonably consistent between the
geographical regions, with some month-to-month variability across time. The greater
variability between regions apparent in the earlier months' data is likely to be
attributable to poorer historical data quality in CAPTND.  

<Div custom-style = "Table or chart caption">Figure 11: First contact DNA rate by health board region, `r date_label_5qt` to `r date_label`^a^</Div>
...
Figure 12 shows the first contact DNA rate for Scotland as a whole in the quarter
...
could be one. It appears from figure 12 that relative deprivation is correlated with
...
<Div custom-style = "Table or chart caption">Figure 12: First contact DNA rate by SIMD quintile, NHSScotland, quarter ending `r date_label`</Div>
...
they may not be appropriate for CAMHS because they do not meet the National Referral
...
# Further Information
Further information and data for this publication are available from the [publication page]`r paste0("(https://publichealthscotland.scot/publications/child-adolescent-and-psychological-therapies-national-dataset-captnd/", dataset_choice, "-in-captnd-4-march-2025/)")` on our website.

The next release of this publication will be `r next_pub_date`.

# Rate this publication
Let us know what you think about this publication via. the link at the bottom of this [publication page]`r paste0("(https://publichealthscotland.scot/publications/child-adolescent-and-psychological-therapies-national-dataset-captnd/", dataset_choice, "-in-captnd-4-march-2025/)")` on the PHS website.
...
Table 5 presents a comparison of total referral counts in CAMHS
...
As we can see from table 5, this figure varies between health boards, with several
...
will continue to improve over time with ongoing data quality assessment and development.  

<Div custom-style = "Table or chart caption">Table 5: Difference between submitted referral counts and CAPTND pathway referral counts, in CAMHS by health board, quarter ending `r date_label` ^a,b^</Div>

```{r Referrals basic v opti table, ft.align = "left"}

flextable(table_data5, theme_fun = NULL) %>%
  my_ft_format() %>%
  flextable::bold(i = nrow(table_data5)) %>%
  align(j = 2:5, align = "right", part = "all") %>%
  height(height = 0.236, part = "body") %>%
  hrule(rule = "atleast", part = "body") %>%
  width(j = 1, width = 4, unit = "cm") %>%
