...
<Div custom-style = "Bullet 1">
There were a total of `r table_data3[[15,2]]` PT appointments
...
\newpage
<Div custom-style = "Table or chart caption">Table 1: Total referrals to PT by health board, by quarter from `r date_label_5qt` to `r date_label` ^1^</Div>
...
PT services across NHSScotland between `r date_label_5qt` and `r date_label`.
There were `r agesex_total[[1,3]]`
(`r agesex_total[[1,4]]`%) females and `r agesex_total[[2,3]]` (`r agesex_total[[2,4]]`%)
males referred over this period. The sex of the remaining number were recorded as
'not known' or 'not specified' (see [Variables Reported](#variables-reported)
for more information). The number of referrals for females peaked at aged
...
In the quarter ending `r date_label`, PT across NHSScotland
...
## Distribution of PT referrals by ethnicity


<Div custom-style = "Table or chart caption">Figure 2: Total referrals to PT by ethnicity, NHSScotland, quarter ending `r date_label`</Div>
```{r Referrals ethnicity chart, fig.width=7.18, fig.height=3.31, include = TRUE}

knitr::include_graphics(paste0("//PHI_conf/MentalHealth5/CAPTND/CAPTND_shorewise/output/analysis_", data_analysis_latest_date, "/shorewise_publication/data/referrals_by_ethnicity/PT_refs_by_ethnicity.png"))
...
As CAPTND is a dataset under development, the increase since the same quarter in
the previous year may reflect improving data quality, rather than an actual increase
in the referral acceptance rate.  

<Div custom-style = "Table or chart caption">Table 2: Status of referrals to PT by health board, quarter ending `r date_label` ^1,2,3^</Div>
...
In PT the primary reason for referral non-acceptance was recorded as '`r ref_rej_reasons[[1,4]]`'
`r ifelse(ref_rej_reasons[[1,4]] == "Unsuitable", "- meaning that the patient is unsuitable for the service to which they have been referred. A patient may be deemed unsuitable for PT because their needs would be better met in another branch of mental health services, e.g. psychiatry.", "")` (`r ref_rej_reasons[[1,7]]`%). The next most common reason for
referral non-acceptance was recorded as '`r ref_rej_reasons[[2,4]]`'
`r ifelse(ref_rej_reasons[[2,4]] == "Unsuitable", "- meaning that the patient is unsuitable for the service to which they have been referred. A patient may be deemed unsuitable for PT because their needs would be better met in another branch of mental health services, e.g. psychiatry.", "")` (`r ref_rej_reasons[[2,7]]`%).  
...
Figure x shows the source of referrals for people referred to PT services across 
NHSScotland for the quarter ending `r date_label`. The most common referral source to PT services was '`r ref_source[[1,4]]`' (`r ref_source[[1,7]]`%), followed by '`r ref_source[[2,4]]`' (`r ref_source[[2,7]]`%).

<Div custom-style = "Table or chart caption">Figure : Top sources of PT referrals, NHSScotland, quarter ending `r date_label`</Div>
```{r Referral source chart, fig.width=7.18, fig.height=3.31, include = TRUE}

knitr::include_graphics(paste0("//PHI_conf/MentalHealth5/CAPTND/CAPTND_shorewise/output/analysis_", data_analysis_latest_date, "/shorewise_publication/data/referrals_by_ref_reason/PT_refs_by_ref_source.png"))

```

## PT total appointment DNA rate

Table x shows the rate of total appointment DNAs recorded in CAPTND for PT in
the latest quarter, for each health board and for NHSScotland overall. Data for 
previous quarters
can be seen in the accompanying [data files]`r paste0("(https://publichealthscotland.scot/publications/child-adolescent-and-psychological-therapies-national-dataset-captnd/", dataset_choice, "-in-captnd-3-september-2024/)")`.

Of the `r dna_rate[[5,5]]` PT appointments recorded for the quarter ending 
...
<Div custom-style = "Table or chart caption">Table : Total PT appointment DNA rate by health board, quarter ending `r date_label` ^1,2,3^</Div>
...
## PT appointments by care location

Figure x shows the most common PT appointment care locations across NHSScotland for the quarter ending `r date_label`. The care location recorded for the majority of PT appointments was '`r appt_loc[[1,3]]`' (`r appt_loc[[1,7]]`%). The next most common care location recorded was '`r appt_loc[[2,3]]`' (`r appt_loc[[2,7]]`%). Care location data was missing for `r appt_loc_missing_fig[[1,7]]`% (`r appt_loc_missing_fig[[1,5]]`) of total PT appointments across NHSScotland for the most recent quarter.

<Div custom-style = "Table or chart caption">Figure : Top care locations of PT appointments, NHSScotland, quarter ending `r date_label`</Div>
...
Figure x shows the most common professional groups conducting PT appointments across NHSScotland for the quarter ending `r date_label`. The professional group most commonly recorded as conducting PT appointments was '`r prof_group[[1,3]]`' (`r prof_group[[1,7]]`%), followed by '`r prof_group[[2,3]]`' (`r prof_group[[2,7]]`%). Professional group data was missing for `r prof_group_missing_fig[[1,7]]`% (`r prof_group_missing_fig[[1,5]]`) of total PT appointments across NHSScotland for the most recent quarter.

<Div custom-style = "Table or chart caption">Figure : Top professional groups conducting PT appointments, NHSScotland, quarter ending `r date_label`</Div>
```{r Professional group chart, fig.width=7.18, fig.height=3.31, include = TRUE}

knitr::include_graphics(paste0("//PHI_conf/MentalHealth5/CAPTND/CAPTND_shorewise/output/analysis_", data_analysis_latest_date, "/shorewise_publication/data/appointments_loc/PT_apps_by_prof_top5.png"))

```

## PT appointment attendance

Table 3 shows the total number of appointments made within PT and recorded 
...
Of the `r figs_apps$"Total appointments"` PT appointments in the
...
to treatment start time. The PT DNA rate across NHSScotland in the
...
<Div custom-style = "Table or chart caption">Table 3: Total and first contact appointments in PT by health board, quarter ending `r date_label`, with DNA rate for first contact appointments ^1,2^</Div>
...
There is some evidence for regional variation in DNA rate, though within the geographical regions
...
region, with the North region fluctuating in between them.
...
knitr::include_graphics(paste0("//PHI_conf/MentalHealth5/CAPTND/CAPTND_shorewise/output/analysis_", data_analysis_latest_date, "/shorewise_publication/data/appointments_att/dna_rate_trend_region_PT.png"))
...
knitr::include_graphics(paste0("//PHI_conf/MentalHealth5/CAPTND/CAPTND_shorewise/output/analysis_", data_analysis_latest_date, "/shorewise_publication/data/appointments_att/dna_simd_plot_last_qt_PT.png"))
...
track an individual’s care pathway in PT. In the absence of
these key variables, the pathway cannot be created in CAPTND.  

Table 4 presents a comparison of total referral counts in PT
...
referrals to PT were submitted to CAPTND, however,
...
<Div custom-style = "Table or chart caption">Table 4: Difference between submitted referral counts and CAPTND pathway referral counts, in PT by health board, quarter ending `r date_label` ^1,2^</Div>
