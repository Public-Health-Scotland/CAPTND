...
update_mmi_dt_wording_comp <- function(wb, time_period){
  ...
  paras <- c(" referrals by health board of treatment and biological sex, month ending", 
             ...
             " group or individual treatment intervention by health board of treatment, month ending",
             " total number of discharge records by health board of treatment, month ending")
  ...
  vec_tabs <- c(#"Cover", 
    ...
    "Tab 8", "Tab 9", "Tab 10", "Tab 11", "Tab 12", "Tab 13", "Tab 14", "Tab 15", "Tab 16")
  for(i in 1:length(vec_tabs)){
  ...
  vec_tabs <- c("Tab 3", "Tab 10", "Tab 11", "Tab 16")
  for(i in 1:length(vec_tabs)){
    writeData(wb, vec_tabs[i], x = ex_period, startCol = 2, startRow = 21, headerStyle = style_text)
    
  }
  
  vec_tabs <- c("Tab 2", "Tab 14")
  for(i in 1:length(vec_tabs)){
    writeData(wb, vec_tabs[i], x = ex_period, startCol = 2, startRow = 20, headerStyle = style_text)
    
  }
  
  vec_tabs <- c("Tab 8", "Tab 15")
  for(i in 1:length(vec_tabs)){
    writeData(wb, vec_tabs[i], x = ex_period, startCol = 2, startRow = 29, headerStyle = style_text)
