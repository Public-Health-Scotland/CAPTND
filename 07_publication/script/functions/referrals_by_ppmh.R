...
df_single_row <- read_parquet(paste0(root_dir,'/swift_glob_completed_rtt.parquet')) |>
  ...
  arrange(!!sym(postcode_o), .by_group = TRUE) |> #order so that referral record is first in pathway
  slice_head(n = 1) |> #only want to keep ppmh at referral
  ungroup() |>  
  add_sex_description() |> 
