...
id_next_pub_date_dmy <- function(latest_month){ # = month_end (last data month)
  ...
  next_pub_date <- data.frame(dates = date_seq) %>% # set as data.frame
    ...
    mutate(day = as.numeric(format(dates, format = "%d")), # get day number
           ...
           desc = paste0(day, ord_suffix, " of ", month, " ", year),
           dates = format(dates, format = "%d/%m/%Y")) %>% # combine for date description
    select(dates) %>% 
    pull() %>% 
    return()
