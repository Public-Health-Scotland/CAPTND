###################################################################.
#### DNA Focused Publication - Total DNA Chart by SIMD and Sex ####.
###################################################################.

# Author: Luke Taylor
# Date: 2026-06-25

create_bar_chart_tot_dna_simd_sex <- function(dataset_choice){
  
  last_pub_period_dna_simd_sex <- read_parquet(paste0(shorewise_pub_data_dir, "/appointments_att/total_dnas_qt_hb_simd_sex.parquet")) |> 
    ungroup() |> 
    ...
    distinct() |>
    group_by(!!sym(dataset_type_o), !!sym(hb_name_o), simd2020_quintile, !!sym(sex_reported_o)) |>
