...
total_appts_quarter_weekday <- function(df){
  ...
  df_app_qt_weekday_sex <- read_parquet(paste0(apps_att_dir, "total_dnas_tot_appts_df.parquet")) |> 
    ...
    bind_rows(summarise(.,
                        ...
                        across(!!sym(hb_name_o), ~"NHS Scotland"),
                        .groups = "drop")) |>
    left_join(df_tot_app_qt_sex, by = c("dataset_type", "hb_name", "day_of_week", 
                                        "sex_reported", "app_quarter_ending")) |>
    mutate(!!sym(hb_name_o) := factor(!!sym(hb_name_o), levels = level_order_hb),
           app_quarter_ending = as.Date(app_quarter_ending, "%Y-%m-%d"),           
           prop_apps_att = round(dna_count/total_apps*100, 1)) |>
    arrange(!!dataset_type_o, !!hb_name_o, !!app_month_o)  |> 
    save_as_parquet(path = paste0(apps_att_dir, measure_label, "weekday_sex_all_hb"))
