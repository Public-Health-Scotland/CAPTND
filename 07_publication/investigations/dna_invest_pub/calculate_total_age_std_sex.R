######################################################################.
#### DNA Focused Publication - Age Standardised Total DNAs by Sex ####.
######################################################################.

# Author: Luke Taylor
# Date: 2026-07-28

age_std_tot_appt_dna_sex <- function(df, total_std_pop){
  ...
  #dna rate by age, sex
  tot_mth_age_sex <- updated_age_groups_df |>
  ...
  age_std_tot_dna_age_sex <- tot_mth_age_sex |>
    ...
    summarise(std_rate_by_sex = round(sum(st_dna_rate, na.rm = TRUE), 1),
              .groups = "drop") |>
    save_as_parquet(paste0(apps_att_dir, measure_label, "qt_hb_age_std_sex"))
