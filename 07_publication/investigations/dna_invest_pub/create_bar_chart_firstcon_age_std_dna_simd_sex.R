#######################################################################################.
#### DNA Focused Publication - First Contact DNA Chart - SIMD and Sex, Std. by Age ####.
#######################################################################################.

# Author: Luke Taylor
# Date: 2026-07-31

create_bar_chart_firstcon_dna_simd_sex_std_age <- function(dataset_choice){
  ...
  ggplot(plot_data, aes(x = simd2020_quintile, y = std_rate_by_simd_sex, fill = sex_reported)) +
    ...
    labs(
      x = "SIMD Quintile",
      y = "First contact age standardised DNA risk (%)",
  ...
  ggsave(paste0(shorewise_pub_data_dir, "/appointments_firstcon/age_std_dna_simd_sex_", dataset_choice, ".png"),
