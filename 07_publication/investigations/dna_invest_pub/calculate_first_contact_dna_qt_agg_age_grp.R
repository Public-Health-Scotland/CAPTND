#########################################################################.
#### DNA Focused Publication - First contact DNAs by Quarter and Age ####.
#########################################################################.

# Author: Luke Taylor
# Date: 2026-06-25

firstcon_appt_quarter_agg_age <- function(df){
  ...
  df_app_qt_agg_age_group <- updated_age_groups_df |> 
    ...
    save_as_parquet(paste0(apps_firstcon_dir, measure_label, "qt_hb_agg_age_group"))
  ...
  df_app_mth_agg_age_group <- updated_age_groups_df |> 
    ...
    arrange(!!dataset_type_o, !!hb_name_o, !!app_month_o) |> 
    #filter(Attendance == 'Patient DNA') |>
    save_as_parquet(paste0(apps_firstcon_dir, measure_label, "mth_hb_agg_age_group"))
