...
###########################################.
### Make Product 1 plot for publication ###
###########################################.

# Author: Bex Madden (adapted from 05_data_quality/product1.R)
# Date: 27/06/2024


make_prod1_forpub <- function() {
  ...
  phs_colours <- c("90 to 100%" = "#80BCEA",# blue 50%
                             "70 to 89.9%" = "#655E9D",# purple 80%
                             "0 to 69.9%" = "#AF69A9") # magenta 80%
  ...
  product1_plot <- df_all %>% 
    ...
    scale_fill_manual(values = phs_colours, name = 'Retained rows', drop = FALSE)+
  ...
  ggsave(paste0(publication_dir,'/prod1_forpub.png'),
         width=29,
         height=13.5,
         units='cm',
         dpi = 300,
