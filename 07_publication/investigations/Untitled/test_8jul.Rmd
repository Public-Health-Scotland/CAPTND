...
table_data4 <- read_parquet("//PHI_conf/MentalHealth5/CAPTND/CAPTND_shorewise/output/analysis_2024-06-28/shorewise_publication/data/apps_total_nr_last_qt.parquet")
...
```{r table-data5, include = FALSE}
table_data5 <- read_parquet("//PHI_conf/MentalHealth5/CAPTND/CAPTND_shorewise/output/analysis_2024-06-28/shorewise_publication/data/apps_first_att_qt_dna_rate.parquet")

if(dataset_choice == "CAMHS"){
  table_data5 <- table_data5 |>
    filter(dataset_type == "CAMHS")
} else {
  table_data5 <- table_data5 |>
    filter(dataset_type == "PT")
...
flextable(table_data3, theme_fun = NULL) %>%
  ...
  hrule(rule = "atleast", part = "body") %>%
  set_table_properties(layout = "autofit")  %>%
  add_header_row(values = c(" ", "Non-acceptance Reason", "Non-acceptance Action"),
                 colwidths = c(2, 2, 2), top = TRUE) %>%
  set_caption(caption = paste0("Table 3: Details of non-accepted referrals to ", dataset_choice, " by NHS board, Quarter ending ", month_end))
...
flextable(table_data4, theme_fun = NULL) %>%
  ...
  hrule(rule = "atleast", part = "body") %>%
  width(width = 1.26) %>%
  set_caption(caption = paste0("Table 4: Total appointments in ", dataset_choice, " by NHS board, with proportions of New vs Return appointments, Quarter ending ", month_end))
...
<Div custom-style = "Table or chart caption">Table or chart caption</Div>
```{r flextable-table-data5, ft.align = "left"}
...
flextable(table_data5, theme_fun = NULL) %>%
  ...
  hrule(rule = "atleast", part = "body") %>%
  set_table_properties(layout = "autofit")  %>%
  set_caption(caption = paste0("Table 5: Rate of 'Did Not Attend' (DNA) appointments ", dataset_choice, " by NHS board, for the five quarters preceding ", month_end))
