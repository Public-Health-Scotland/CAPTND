...
calculate_pat_waiting_monthly_clean <- function(df){
  
################################################ load dataframe #######################################################
  df_select <- read_parquet(paste0(root_dir,'/swift_glob_completed_rtt.parquet')) |>
...
######################## identify cases waiting greater than 18 months since last contact #############################
  last_contact_df <- df_select |>
    group_by(across(all_of(data_keys))) |> 
...
########################## identify cases with no contact waiting greater than 24 months ###############################
  no_contact_df <- df |>
    group_by(across(all_of(data_keys))) |> 
    filter(is.na(!!sym(case_closed_date_o)) & is.na(!!sym(act_code_sent_date_o))
...
############################################ create csv/parquet files ##################################################
  ...
  save_as_parquet(table_wait_summary, paste0(patients_waiting_dir,'/by_month/monthly_waits_patients_cleaned'))
  
  write_csv_arrow(table_wait_summary, paste0(patients_waiting_dir,'/nPatients_waitingCleaned_subSource_monthly.csv'))
