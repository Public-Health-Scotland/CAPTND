...
df_rtt <- df |>
  ...
  save_as_parquet("../../../data/RTT_testing/RTT_process_data.parquet")
rm(df_rtt)
gc()
message('DF ready, calculating clock reset\n')

# DNA/CNA/CNW logic - adjusting the clock start date to account for resets   
df_reset <- read_parquet("../../../data/RTT_testing/RTT_process_data.parquet") |> 
...
df_reset <- df_reset |>
  ...
  distinct()  |> # save out here?
  save_as_parquet("../../../data/RTT_testing/RTT_process_data.parquet")

gc() # removes duplicates 
...
df_rtt_complete <-  read_parquet("../../../data/RTT_testing/RTT_process_data.parquet") |> 
