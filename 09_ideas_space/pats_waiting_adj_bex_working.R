...
df_rtt <- df |> 
  ...
  arrange(!!!syms(c(dataset_type_o, hb_name_o, ucpn_o, app_date_o))) |>    # will have to do a cross join with the months so pathways with no submission in a month will continue to 'accrue' waiting time
  
  # calculate basic unadjusted RTT
  mutate(pat_wait_unadj = case_when(
    first_treat_app >= sub_month_end ~ as.integer(sub_month_end - ref_rec_date_opti), # if treatment starts AFTER the end of a given sub month, unadj wait time. 
    TRUE ~ NA_integer_)) |> # maybe not necessary to create unadj in the patients waiting version...?
...
df_reset <- df_rtt |>
  mutate(dna_date = if_else(
    att_status %in% c(3, 5, 8) &
      app_date < first_treat_app, # this maybe needs scrapped in waiting version??? or keep as just before treatment start?
...
df_rtt_complete <- df_rtt |>
  ...
  left_join(df_reset, by = c(all_of(data_keys), "app_date")) |> # appends new clock start date to complete data # is app date necessary here? should be 1clock start per pathway
  ...
  mutate(clock_start = case_when(is.na(clock_start) ~ ref_rec_date,
         ...
         guarantee_date = clock_start + 126)#, # make new guarantee date relative to the clock_start ###GOT THIS FAR 4/10/24 # UNAV BEFORE FIRST TREATMENT SHOULD BE OK??/
